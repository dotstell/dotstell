          ...
          <div style={{ maxWidth: 660 }}>
            ...
            <Section id="ai" title="✨ AI features">
              ...
              <Tip icon="🔌" title="Ollama on the live app — Local AI Agent"
                body={<>When using <strong>Ollama</strong> from <strong>dotstell.app</strong> (the hosted version), browsers block direct connections to localhost for security reasons (Private Network Access spec). Run the <strong>Dotstell Local AI Agent</strong> to bridge the gap — it's a tiny Node.js proxy that adds the required headers and forwards requests to your local Ollama. Start it with: <code style={{ fontSize: 11, backgroundColor: 'rgba(0,0,0,0.15)', padding: '1px 5px', borderRadius: 3 }}>node packages/agent/index.mjs</code> (from the repo root). The AI Settings modal shows a green <strong>"Local Agent is running"</strong> badge when detected. You do not need the agent when running dotstell locally at localhost:3000.</>}
              />
