...
export default function LandingPage() {
  return (
    // html/body are globally locked (see auth/layout.tsx for the full reason) so the
    // document itself never scrolls — this page needs its own scroll container instead,
    // same as AppLayout's <main>, or nothing past the first viewport is reachable.
    <div style={{ height: '100dvh', overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain', display: 'flex', flexDirection: 'column', background: 'var(--background)', color: 'var(--foreground)' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {/* About/GitHub are secondary on a marketing nav; Sign in / Get started are the
              two actions that actually matter. Below 480px (an actual phone, not just a
              narrow window) there is not room for all four without wrapping or clipping —
              confirmed by screenshot: "Get started free" was rendering partly off-screen.
              Hiding the two lower-priority links via CSS keeps this a server component. */}
          <a href="https://www.dotstell.com/" target="_blank" rel="noopener noreferrer" className="landing-nav-secondary" style={{ padding: '6px 12px', fontSize: 13, borderRadius: 6, color: 'var(--muted-foreground)', textDecoration: 'none' }}>About</a>
          <a href="https://github.com/dotstell/dotstell" target="_blank" rel="noopener noreferrer" className="landing-nav-secondary hover:text-[var(--foreground)] transition-colors" style={{ padding: '6px 12px', fontSize: 13, borderRadius: 6, color: 'var(--muted-foreground)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 5 }}>
